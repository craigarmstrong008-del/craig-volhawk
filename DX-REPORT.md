...
**Project:** Craig VolHawk — Volatility-Triggered OCO Agent  
...
```
The docs reference `price` as a field. The actual field is `usdPrice`. This caused a silent `undefined` bug on first use — not a crash, just wrong values. Docs should match the live response shape exactly.
...
`POST /trigger/v2/orders/price` exists in the reference. The auth challenge flow is described. But the actual JSON body shape for an OCO order — what fields, what types, how to specify the paired TP and SL prices — is not in the docs I could find.
...
At 0.5 RPS, I can only query the Price API every 2 seconds per IP. With 3 tokens, that's 6 seconds per full cycle minimum — fine for a slow monitor, but I found out about this limit from `llms.txt`, not from any response header or error message when I hit it.
...
**What works:** The skills surface the right starting points — which endpoints exist, what the auth flow looks like, what the key concepts are.
...
### Jupiter CLI
I installed it. The JSON-native design is correct — outputting clean JSON makes it trivially pipeable into other tools or readable by an LLM.
...
**1. Ship every endpoint with a live "Try it" that shows the real response.** Not a static code block — an actual embedded request that runs against the API and returns live data. The fastest way to understand an API is to see it work. Every minute spent reading static docs is a minute not spent building.
...
**3. Trigger docs need a full OCO walkthrough.** Auth challenge → sign message → submit order → poll status → order fills → repeat. The individual pieces exist. The end-to-end sequence does not. For the most complex (and most useful) API on the platform, this is the gap that will lose the most developers at step 3 of 5.
...
- **Batch order status endpoint.** `GET /trigger/v2/orders?wallet=...&status=open` — currently I can't efficiently poll the status of all my agent's open orders without individual requests per order.

- **Agent identity / API key linkage.** Right now, Trigger orders require a wallet signature to verify ownership. For pure LLM agents that don't control a keypair natively, there's no path to autonomous order submission without a wallet-holding middleware layer. A lightweight "agent API key → wallet delegation" mechanism — where a human pre-authorizes an API key to place orders up to a size limit on their behalf — would make fully autonomous trading agents viable on Jupiter without requiring every agent to hold a private key.
...
| Area | Score | Notes |
|---|---|---|
| Time to first call | ✅ Excellent | ~4 min, keyless, no friction |
| `llms.txt` quality | ✅ Excellent | Best in class for LLM consumption |
| Price API docs | ⚠️ Good | Response field names don't match docs (`usdPrice` vs `price`) |
| Trigger API docs | ❌ Incomplete | OCO payload shape not documented; no simulation mode |
| Error messages | ✅ Good | Validation errors are specific and actionable |
| Rate limit signalling | ⚠️ Missing | No response headers; silent failures without `llms.txt` |
| AI Stack (Skills/CLI/MCP) | ⚠️ Promising | Right direction, needs code examples and CLI/API field parity |
| Agent-native features | ❌ Gap | No simulation mode; wallet delegation model missing |
